...
def detect_ring_systems_for_viz(block_text: str) -> List[dict]:
    ...
    try:
        ...
        ring_systems = []
        for i, m in enumerate(moieties):
            color = _RING_SYSTEM_COLORS[i % len(_RING_SYSTEM_COLORS)]
            patches = patches_by_system.get(m.label, [])
            sys_atom_names = [mol.GetAtomWithIdx(int(idx)).GetProp("_TriposAtomName") if mol.GetAtomWithIdx(int(idx)).HasProp("_TriposAtomName") else mol.GetAtomWithIdx(int(idx)).GetSymbol() for idx in m.atom_indices]
            patch_info = []
            for p in patches:
                patch_info.append({"label": p.label, "atom_names": [mol.GetAtomWithIdx(int(idx)).GetProp("_TriposAtomName") if mol.GetAtomWithIdx(int(idx)).HasProp("_TriposAtomName") else mol.GetAtomWithIdx(int(idx)).GetSymbol() for idx in p.atom_indices]})
            ring_systems.append({
                "label": m.label, "atom_indices_0based": list(m.atom_indices),
                "n_atoms": len(m.atom_indices), "n_patches": len(patches),
                "color": color, "atom_names": sys_atom_names, "patches": patch_info,
            })
        return ring_systems
...
document.addEventListener("DOMContentLoaded", () => {{
  ...
  if (RING_SYSTEMS.length > 0) {{
    html += `<div class="section overlay-section rs-section" style="display:none;"><div class="section-title">Aromatic Ring Systems (${{RING_SYSTEMS.length}})<span class="rdkit-badge">RDKit</span></div><div class="rs-legend">`;
    RING_SYSTEMS.forEach(rs => {{
      ...
      html += `<div class="rs-item-block"><div class="rs-item"><div class="rs-swatch" style="background:${{rs.color}};opacity:0.7;"></div><span class="rs-label">${{rs.label.replace('ring_system_', 'RS')}}</span><span class="rs-detail">${{rs.n_atoms}} atoms &middot; ${{rs.n_patches === 1 ? '1 ring' : rs.n_patches + ' rings'}}</span></div>${{atomNamesHtml}}</div>`;
    }});
    html += `</div></div>`;
  }}
