...
def _bond_in_ring(atoms: Dict[int, dict], u: int, v: int) -> bool:
    """Return True if the bond u–v is part of a ring (i.e. is not a bridge).

    A bond is cyclic if and only if removing it still leaves u and v connected
    through an alternative covalent path.  The check is purely graph-based
    (BFS from u to v skipping the direct u→v step) and requires no extra data
    beyond the bond adjacency lists already stored in ``atoms``.
    """
    visited: Set[int] = {u}
    q: deque[int] = deque([u])
    while q:
        ...
        for nb in atoms[node]["bonds"]:
            ...
            if nb == v:
                return True  # alternative path found → ring bond
...
def _path_contains_ring_bond(atoms: Dict[int, dict], path: List[int]) -> bool:
    """Return True if any step in *path* traverses a ring bond.

    When the shortest covalent path from donor D to acceptor A crosses a ring
    bond, the D···A proximity is geometrically locked by the rigid ring
    framework.  Donor and acceptor are effectively constrained to be close
    (or not close) regardless of molecular conformation — the apparent H-bond
    contact is a structural artefact, not a conformationally driven interaction.

    Typical cases excluded by this filter:

    * Ortho-disubstituted aromatic rings (path crosses 1 ring bond).
    * Substituents on adjacent atoms of a fused bicyclic system (path crosses
      the inter-ring fusion bond and/or an intra-ring bond).
    * Meta- and para-disubstituted rings (path crosses 2–3 ring bonds, geometry
      similarly locked by the ring framework).
    """
    for i in range(len(path) - 1):
        if _bond_in_ring(atoms, path[i], path[i + 1]):
            return True
    return False
...
def find_imhbs(
    ...
    Filters applied (in order):
      ...
      4. Ring-path filter: if the shortest covalent D→A path crosses any ring
         bond, the D···A proximity is geometrically locked by the ring framework
         (not conformationally driven) — contact is discarded.
    """
    ...
    for donor_id, h_id in donor_h_pairs:
        for acc_id in acceptors:
            ...
            # Ring-path filter: if the shortest D→A covalent path crosses any
            # ring bond, the D···A proximity is locked by ring planarity and is
            # not a conformationally driven IMHB.  Discard the contact.
            if cov_path and _path_contains_ring_bond(atoms, cov_path):
                continue
