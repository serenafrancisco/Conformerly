        ...
        if do_pi:
            try:
                if _HAS_PI:
                    ...
                    pi_rows, _, _ = compute_pair_outputs(
                        mol=mol, rings=rings,
                        min_bond_sep=2, use_pi_criteria=True, pi_params=pi_params,
                    )
                    # Build a lookup from ring label -> centroid for 3D rendering
                    ring_centroids = {
                        r.label: _centroid(coords_for_atoms(mol, r.atom_indices)).tolist()
                        for r in rings
                    }
                    pi_json = []
                    for r in pi_rows:
                        ...
                        ca = ring_centroids.get(pa_label)
                        cb = ring_centroids.get(pb_label)
                        if ca and cb:
                            pi_json.append({
                                "patch_a": pa_label, "patch_b": pb_label,
                                "pi_class": r.get("pi_class", ""),
...
function updatePanel(idx) {{
  ...
  if (pis.length > 0) {{
    html += `<table class="int-table"><tr><th>Rings</th><th>Class</th><th>d(c&middot;&middot;&middot;c)</th><th>&ang;planes</th></tr>`;
    pis.forEach(pi => html += `<tr><td>${{pi.patch_a}} &harr; ${{pi.patch_b}}</td><td><span class="${{pi.pi_class === 'face-to-face' ? 'tag-ff' : 'tag-ef'}}">${{pi.pi_class}}</span></td><td>${{pi.centroid_dist.toFixed(2)}} \u00C5</td><td>${{pi.plane_angle.toFixed(1)}}&deg;</td></tr>`);
    html += `</table>`;
